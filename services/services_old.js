...
// Assigning the cachFactory to 'myCache'
    ...
    .service('parseService', function (apiService, logicService) {
        ...
        self.categories_with_array = ["films", "pilots", "characters", "planets", "starships", "species", "vehicles"];
        ...
        self.parseResults = function(results_to_parse, category) {
            ...
            console.log('in parse results function');
            ...
            var processResults = function(category, result_array) {

                var result_array = result_array;

                switch (category) {
                    case "people":
                        var homeworld_name = result_array.data.name;
                        self.homeworlds.push({
                            name: result_array.data.name,
                            url: result_array.data.url
                        });
                        break;
                    case "vehicles":
            ...
            var processArrayResults = function () {}
            ...
            results_to_parse.forEach(function(result_item, i, arr) {
                    ...
                    if (result_item[category]) {
                        ...
                        if (!cache_results) {
                            apiService.getDataUrl(url, function(response) {
                                ...
                                var processed_results = processResults(category, response);
                                // switch (category) {
                                //     case "people":
                                ...
                                logicService.setCacheItem(url, processed_results);
                            }, function(err) {
                                console.log(err.status);
                            });
                        } else {

                            processResults(category, cache_results);
                ...
                self.categories_with_array.forEach(function(category) {
                    ...
                    if (result_item[category]) {
                        ...
                        processArrayResults(category, result_item_a_length, a_index);
                        // console.log(category);
                        // switch (category) {
                        //     case 'films':
